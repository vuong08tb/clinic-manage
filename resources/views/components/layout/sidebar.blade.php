@php
    $groups = [
        [
            'label' => 'Tổng quan',
            'items' => [
                ['label' => 'Dashboard', 'icon' => 'dashboard', 'href' => route('dashboard'), 'active' => request()->routeIs('dashboard'), 'permissions' => [], 'ready' => true],
            ],
        ],
        [
            'label' => 'Tiếp đón',
            'items' => [
                ['label' => 'Bệnh nhân', 'icon' => 'patients', 'permissions' => ['PATIENTS.FINDALL'], 'ready' => false],
                ['label' => 'Lịch hẹn', 'icon' => 'calendar', 'permissions' => ['APPOINTMENTS.FINDALL'], 'ready' => false],
            ],
        ],
        [
            'label' => 'Khám bệnh',
            'items' => [
                ['label' => 'Phiếu khám', 'icon' => 'examination', 'permissions' => ['EXAMINATIONS.FINDALL'], 'ready' => false],
                ['label' => 'Toa thuốc', 'icon' => 'prescription', 'permissions' => ['PRESCRIPTIONS.FINDALL', 'PRESCRIPTIONS.CREATE'], 'ready' => false],
            ],
        ],
        [
            'label' => 'Dược',
...
<aside
    ...
    :class="{ 'translate-x-0': $store.ui.sidebarOpen }"
...
>
    <div class="flex h-16 items-center justify-between border-b border-slate-100 px-4">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 rounded-lg">
            <span class="grid h-9 w-9 place-items-center rounded-xl bg-blue-600 text-white shadow-sm shadow-blue-600/20">
    ...
    <nav class="flex-1 space-y-6 overflow-y-auto px-3 py-5">
        @foreach ($groups as $group)
            ...
            @endphp
            <section
                @if ($groupPermissions !== []) x-show="$store.auth.canAny(@js($groupPermissions))" @endif
            >
                ...
                <div class="mt-2 space-y-1">
                    @foreach ($group['items'] as $item)
                        @if ($item['ready'])
                            <a
                                href="{{ $item['href'] }}"
                                @if ($item['permissions'] !== []) x-show="$store.auth.canAny(@js($item['permissions']))" @endif
                                @class([
                        ...
                        @else
                            <span
                                x-show="$store.auth.canAny(@js($item['permissions']))"
