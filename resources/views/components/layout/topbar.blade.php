<header
    class="sticky top-0 z-20 flex h-16 items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6">
    <button type="button" class="mr-3 rounded-xl p-2 text-slate-600 hover:bg-slate-100 lg:hidden"
        x-on:click="$store.ui.toggleSidebar()" aria-label="Mở thanh điều hướng">
        <x-ui.icon name="menu" />
    </button>
    ...
    <div class="flex items-center gap-2">
        <button type="button" class="relative rounded-xl p-2.5 text-slate-500 hover:bg-slate-100 hover:text-slate-800"
            aria-label="Thông báo" x-on:click="$store.ui.notify('Trung tâm thông báo sẽ được triển khai ở task sau.')">
            <x-ui.icon name="bell" />
        </button>

        <div class="relative" x-data="{ open: false }" x-on:click.outside="open = false">
            <button type="button" class="flex items-center gap-2 rounded-xl p-1.5 pr-2 hover:bg-slate-100"
                x-on:click="open = !open" :aria-expanded="open" aria-haspopup="menu">
                <span class="grid h-8 w-8 place-items-center rounded-lg bg-blue-100 text-xs font-bold text-blue-700"
                    x-text="$store.auth.initials"></span>
            ...
            <div x-cloak x-show="open" x-transition.origin.top.right
                ...
                <button type="button"
                    ...
                    x-on:click="$store.auth.logout()" role="menuitem">
