...
@section('content')
    <div x-data="dashboardPage" x-init="init()" class="mx-auto max-w-[1600px] space-y-7">
                    ...
                    <article class="surface-card p-5">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <div x-show="widget.loading" class="mt-3 h-9 w-20 animate-pulse rounded-lg bg-slate-100"></div>
                                <p x-show="!widget.loading" class="mt-2 text-3xl font-bold tracking-tight text-slate-950" x-text="widgetValue(widget)"></p>
                            </div>
                            <span
                                class="grid h-11 w-11 place-items-center rounded-xl text-sm font-bold"
                                :class="{
                                ...
                                x-text="widget.label.charAt(0)"
                                aria-hidden="true"
                            ></span>
                        </div>
                        <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                            <p class="text-xs text-slate-500" x-text="widget.error ? 'Không tải được dữ liệu' : widget.caption"></p>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
            <section class="surface-card overflow-hidden" x-show="$store.auth.can('APPOINTMENTS.FINDALL')">
                <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-6">
                    ...
                    <span class="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700" x-text="`${appointments.length} lịch`"></span>
                </div>
                ...
                <div x-cloak x-show="!appointmentsLoading && !appointmentsError && appointments.length > 0" class="overflow-x-auto">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <template x-for="appointment in appointments" :key="appointment.id">
                                <tr class="hover:bg-slate-50/70">
                                    <td class="whitespace-nowrap px-5 py-4 text-sm font-semibold text-slate-900 sm:px-6" x-text="formatTime(appointment.scheduled_at)"></td>
                                    <td class="px-5 py-4 sm:px-6">
                                        <p class="text-sm font-semibold text-slate-800" x-text="appointment.patient?.full_name ?? 'Chưa có thông tin'"></p>
                                        <p class="mt-0.5 max-w-xs truncate text-xs text-slate-500" x-text="appointment.reason || 'Không ghi lý do'"></p>
                                    </td>
                                    <td class="hidden px-5 py-4 text-sm text-slate-600 md:table-cell sm:px-6" x-text="appointment.doctor?.user?.name ?? '—'"></td>
                                    <td class="whitespace-nowrap px-5 py-4 sm:px-6">
                                        <span
                                            ...
                                            x-text="statusLabel(appointment.status)"
                                        ></span>
                                    </td>
                                </tr>
                            </template>
            ...
            <aside class="surface-card h-fit p-5 sm:p-6">
                ...
                <div class="mt-5 space-y-2">
                    <button
                        x-show="$store.auth.can('APPOINTMENTS.CREATE')"
                        type="button"
                    ...
                    >
                        <span class="grid h-9 w-9 place-items-center rounded-lg bg-blue-100 text-blue-700"><x-ui.icon name="calendar" /></span>
                        <span class="min-w-0 flex-1">
                            <span class="block text-sm font-semibold text-slate-800">Tạo lịch hẹn</span>
                            <span class="block text-xs text-slate-500">Task FE-04</span>
                        </span>
                    ...
                    >
                        <span class="grid h-9 w-9 place-items-center rounded-lg bg-amber-100 text-amber-700"><x-ui.icon name="invoice" /></span>
                        <span class="min-w-0 flex-1">
                ...
                <div class="mt-5 rounded-xl bg-slate-50 p-4">
                    <div class="flex gap-3">
                        <span class="mt-0.5 text-blue-600"><x-ui.icon name="clock" /></span>
                        <div>
                            <p class="text-sm font-semibold text-slate-800">Triển khai theo checklist</p>
                            <p class="mt-1 text-xs leading-5 text-slate-500">Mỗi module sẽ được mở khi hoàn thành đủ loading, error, permission và responsive.</p>
                        </div>
                    </div>
                </div>
            </aside>
