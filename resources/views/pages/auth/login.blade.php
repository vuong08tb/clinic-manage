...
@section('content')
    <main
        class="relative grid min-h-screen overflow-hidden lg:grid-cols-[minmax(0,1fr)_minmax(480px,0.78fr)]"
        x-data="loginPage"
        x-init="init()"
    >
        <section class="relative hidden overflow-hidden bg-blue-700 p-12 text-white lg:flex lg:flex-col lg:justify-between">
            <div class="absolute inset-0 opacity-40" aria-hidden="true">
                ...
                <div class="absolute right-0 bottom-0 h-[32rem] w-[32rem] translate-x-1/3 translate-y-1/3 rounded-full bg-indigo-950/40 blur-3xl"></div>
            </div>
                    ...
                    <form class="mt-8 space-y-5" x-on:submit.prevent="submit" novalidate>
                        <div>
                            ...
                            <input
                                ...
                                placeholder="name@clinic.vn"
                                :aria-invalid="Boolean(fieldError('email'))"
                                aria-describedby="email-error"
                            >
                            <p id="email-error" x-cloak x-show="fieldError('email')" class="mt-1.5 text-sm text-rose-600" x-text="fieldError('email')"></p>
                        </div>
                        ...
                        <div>
                            ...
                            >
                            <p id="password-error" x-cloak x-show="fieldError('password')" class="mt-1.5 text-sm text-rose-600" x-text="fieldError('password')"></p>
                        </div>
