...
20. [Các lệnh thường dùng trong quá trình code](#20-Các-lệnh-thường-dùng-trong-quá-trình-code)
...
### Giới hạn phạm vi (các ràng buộc độc lập nêu trong phần _Mô tả hệ thống_ của đề)

Phần _2. Mô tả hệ thống_ của đề liệt kê một loạt **dòng ràng buộc/giới hạn độc lập** — mỗi dòng là một biên phạm vi, không phải hạng mục sẽ code:
...
> **Ghi chú về Frontend:** đề đặt "Frontend (Blade/Vue/React)" trong nhóm giới hạn phạm vi ở phần _Mô tả hệ thống_ (không phải một mục "cấm" riêng). Trong phạm vi thực tập này ta **chủ động làm thêm** một frontend Blade tối giản (cùng repo) tiêu thụ API — coi là phần mở rộng tự chọn, không thay thế yêu cầu API-first. Xem [mục 14](#14-frontend-blade--vite).
